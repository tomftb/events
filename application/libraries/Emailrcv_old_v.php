...
class Emailrcv {
    ...
    public function getMessages($type = 'text') {
        ...
        //$this->attachments_dir = dirname(__FILE__).$this->slash.rtrim($this->attachments_dir, '/');
        $this->attachments_dir =filter_input(INPUT_SERVER,'DOCUMENT_ROOT').'/'.rtrim($this->attachments_dir, '/');
        log_message('debug', "[".__METHOD__."] ". filter_input(INPUT_SERVER,'DOCUMENT_ROOT')." - ".$this->attachments_dir);
    ...
    public function getFiles($r) { //save attachments to directory
		$pullPath = $this->attachments_dir . '/' . $r['file'];
                log_message('debug', "[".__METHOD__."] ".$pullPath);
                log_message('debug', "[".__METHOD__."] FILE UID - ".$r['uid']);
                log_message('debug', "[".__METHOD__."] FILE PART - ".$r['part']);
		$res = true;
        if (file_exists($pullPath)) {
			$res = false;
        } elseif (!is_dir($this->attachments_dir)) {
            $this->errors[] = 'Cant find directory for email attachments! Message ID:' . $r['uid'].' PULL PATCH - '.$pullPath;
            return false;
		...
		if($res && !preg_match('/\.php/i', $r['file']) && !preg_match('/\.cgi/i', $r['file']) && !preg_match('/\.exe/i', $r['file']) && !preg_match('/\.dll/i', $r['file']) && !preg_match('/\.mobileconfig/i', $r['file'])){
			if (($filePointer = fopen($pullPath, 'w')) == false) {
				...
				return false;
                        ...
                        log_message('debug', "[".__METHOD__."] FILE ENCODING - ".$r['encoding']);
			switch ($r['encoding']) {
				case 3: //base64
				...
				default:
					$streamFilter = null;
                        ...
                        log_message('debug', "[".__METHOD__."] AFTER STREAM SWITCH");
                        log_message('debug', "[".__METHOD__."] STREAM FILTER - ".$streamFilter);
                        log_message('debug', "[".__METHOD__."] FILE POINTER - ".$filePointer);
                        log_message('debug', "[".__METHOD__."] IMAP STREAM - ".$this->imapStream);
			imap_savebody($this->imapStream, $filePointer, $r['uid'], $r['part'], FT_UID); //
                        log_message('debug', "[".__METHOD__."] AFTER IMAP SAVE BODY");
			if ($streamFilter) {
				stream_filter_remove($streamFilter);
			}
			fclose($filePointer);
                        log_message('debug', "[".__METHOD__."] AFTER FILE CLOSE");
    ...
    {
        log_message('debug', "[".__METHOD__."] ".php_uname());
        $sys=strtoupper(substr(PHP_OS, 0, 3));
        if ( $sys === 'WIN')
        {
            log_message('debug', "[".__METHOD__."] OS - WINDOWS");
            $this->slash='\\';
        }
        else if($sys === 'LIN')
        {
            log_message('debug', "[".__METHOD__."] OS - LINUX");
            $this->slash='/';
        }
        else
        {
            log_message('debug', "[".__METHOD__."] OS - ANOTHER");
            $this->slash='/';
        }
    }
